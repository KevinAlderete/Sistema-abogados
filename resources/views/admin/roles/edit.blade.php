            ...
            <div class="bg-indigo-50 overflow-hidden shadow-sm sm:rounded-lg min-h-screen">
                <div class="flex flex-col sm:flex-row justify-between px-5 pt-5">
                    <div class="py-2">
                        ...
                        <li class="text-sm pl-2 capitalize leading-normal before:float-left before:pr-2 before:content-['/']" aria-current="page">Editar rol</li>
                        </ol>
                    </div>
                    <a href="{{ route('admin.roles.index') }}" class="text-white font-bold justify-center px-5 py-2 border-solid flex bg-blue-700 hover:bg-blue-800 rounded-3xl">
                        Atras
                    </a>
                </div>
                ...
                <div class="overflow-hidden rounded-lg border bg-white border-gray-300 shadow-md m-5 p-5">
                    <div class="flex flex-col gap-1 sm:gap-4 sm:flex-row">
                        <form method="POST" class="mt-11 w-full flex flex-col gap-1" action="{{ route('admin.roles.update', $role->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="flex flex-col gap-1 sm:gap-4 sm:flex-row">
