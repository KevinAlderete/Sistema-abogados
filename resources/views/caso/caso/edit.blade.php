            ...
            <div class="bg-indigo-50 overflow-hidden shadow-sm sm:rounded-lg min-h-screen">
                <div class="flex justify-between px-5 pt-5">
                    <div class="py-2">
                        ...
                        <li class="text-sm pl-2 capitalize leading-normal before:float-left before:pr-2 before:content-['/']" aria-current="page">Editar caso</li>
                        </ol>
                    </div>
                ...
                <div class="overflow-hidden rounded-lg border bg-white border-gray-300 shadow-md m-5 p-5">
                    <div class="mt-8 space-y-8">
                        <form method="POST" class="flex flex-col gap-1" action="{{ route('caso.caso.update', $caso->id) }}">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="relative z-0 w-full mb-6 group">
                                        <select name="estado" id="estado" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                                            @if( $caso->estado  == 'En proceso')
                                                <option value="En proceso">{{ $caso->estado }}</option>
                                                <option value="Archivado">Archivado</option>
                                            @else
                                                <option value="Archivado">{{ $caso->estado }}</option>
                                ...
                                <div class="w-full flex flex-col gap-1">
                                    <div class="relative z-0 w-full mb-6 group">
                                        <select name="id_cliente" id="id_cliente" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                                            <option value="" selected disabled>{{$caso->cliente->nombre }} {{$caso->cliente->apellidos }}</option>
                                    ...
                                    <div class="relative z-0 w-full mb-6 group">
                                        <textarea  name="descripcion" class="resize-none block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" id="descripcion" cols="30" rows="6" placeholder="">{{$caso->descripcion }}</textarea>
