        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-indigo-50 overflow-hidden shadow-sm sm:rounded-lg min-h-screen">
                <div class="flex flex-col sm:flex-row justify-between px-5 pt-5">
                    <div class="py-2">
                        ...
                        <ol class="flex flex-wrap pt-1 bg-transparent rounded-lg sm:mr-16">
                            ...
                            <li class="text-sm leading-normal pl-2 before:float-left before:pr-2 before:content-['/'] before:opacity-50">
                                <a class="opacity-50 hover:opacity-100" href="{{ url()->previous() }}">caso</a>
                            </li>
                            <li class="text-sm leading-normal pl-2 before:float-left before:pr-2 before:content-['/'] before:opacity-50">
                ...
                <section class="overflow-hidden rounded-lg border bg-white border-gray-300 shadow-md m-5 p-5">
                    <h1 class="text-xl font-bold text-white capitalize dark:text-white">Documentos</h1>
                    <form method="POST" action="{{ route('caso.caso.file.update', $cas_documentos->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="flex flex-col gap-1 sm:gap-4 sm:flex-row">
                            <div class="w-full flex flex-col gap-1">
                                <div class="relative z-0 w-full mb-6 group">
                                    <input id="tipo_documento" value="{{ $cas_documentos->tipo_documento }}" name="tipo_documento" placeholder="" type="text" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                                ...
                                <div class="relative z-0 w-full mb-6 group">
                                    <textarea name="descripcion" id="descripcion" placeholder="" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer">{{ $cas_documentos->descripcion }}</textarea>
