        ...
        <div class="bg-indigo-50 overflow-hidden shadow-sm sm:rounded-lg min-h-screen">
            ...
            <div class="flex flex-col sm:flex-row justify-between px-5 pt-5">
                <div class="py-2">
                    ...
                    <li class="text-sm pl-2 capitalize leading-normal before:float-left before:pr-2 before:content-['/']" aria-current="page">Conciliadores</li>
                    </ol>
                </div>
                <a href="{{ route('conciliacion.conciliador.create') }}" class="text-white font-bold justify-center px-5 py-2 border-solid flex bg-blue-700 hover:bg-blue-800 rounded-3xl">
                    <span>Añadir conciliador</span>
                </a>
            </div>
                        ...
                        @foreach ($conciliadores as $conciliador)
                            <tr class="hover:bg-gray-50">
                                <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                                    ...
                                    <div class="text-sm">
                                    <div class="font-medium text-gray-700">{{ $conciliador->nombre }}</div>
                                    </div>
                                </th>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end gap-4">
                                        <a href="{{ route('conciliacion.conciliador.edit', $conciliador->id) }}" class="hover:text-blue-500 text-gray-600 hover:scale-110">
                                            <svg
                                        ...
                                        <form class="hover:text-red-500 text-gray-600 hover:scale-110 boton-eliminar" method="POST" action="{{ route('conciliacion.conciliador.destroy', $conciliador->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit">
                                                <svg
            ...
            <div class="overflow-hidden rounded-lg border border-gray-300 bg-white shadow-md m-5 py-3 px-5">
                {{ $conciliadores->links() }}
            </div>
        </div>
    </div>
