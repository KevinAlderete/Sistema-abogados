        ...
        <div class="bg-indigo-50 overflow-hidden shadow-sm sm:rounded-lg min-h-screen">
            ...
            <div class="flex justify-between px-5 pt-5">
                <div class="py-2">
                    ...
                    <li class="text-sm pl-2 capitalize leading-normal before:float-left before:pr-2 before:content-['/']" aria-current="page">Invitados</li>
                    </ol>
                </div>
                <a href="{{ route('conciliacion.invitado.create') }}" class="text-white font-bold px-3 py-2 border-solid flex bg-blue-700 hover:bg-blue-800 rounded-3xl">
                    <span class="pl-2">Añadir invitado</span>
                </a>
            </div>
            ...
            <div class="overflow-hidden rounded-lg border bg-white border-gray-300 shadow-md m-5">
                ...
                <thead class="bg-gray-50">
                    ...
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900 hidden sm:flex">Teléfono</th>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900 text-center">Acciones</th>
                    </tr>
                </thead>
                    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                        @foreach ($con_invitados as $con_invitado)
                            <tr class="hover:bg-gray-50">
                                <th class="px-6 py-4 font-normal text-gray-900">
                                    
                                    <div class="text-sm">
                                    <div class="font-medium text-gray-700">{{ $con_invitado->nombre }}</div>
                                    </div>
                                </th>
                                
                                <td class="px-6 py-4">
                                    {{ $con_invitado->empresa }}    
                                </td>
                                <td class="px-6 py-4 hidden sm:flex">
                                    {{ $con_invitado->telefono }}    
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex justify-end gap-4">
                                        <a href="{{ route('conciliacion.invitado.edit', $con_invitado->id) }}" class="hover:text-blue-500 text-gray-600 hover:scale-110">
                                            <svg
                                        ...
                                        <form class="hover:text-red-500 text-gray-600 hover:scale-110 boton-eliminar" method="POST" action="{{ route('conciliacion.invitado.destroy', $con_invitado->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit">
                                                <svg
