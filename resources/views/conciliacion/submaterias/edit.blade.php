            ...
            <div class="bg-indigo-50 overflow-hidden shadow-sm sm:rounded-lg min-h-screen">
                <div class="flex flex-col sm:flex-row justify-between px-5 pt-5">
                    <div class="py-2">
                        ...
                        <li class="text-sm pl-2 capitalize leading-normal before:float-left before:pr-2 before:content-['/']" aria-current="page">Editar submateria</li>
                        </ol>
                    </div>
                ...
                <div class="overflow-hidden rounded-lg border bg-white border-gray-200 shadow-md m-5 p-5">
                        ...
                        <form method="POST" class="flex flex-col gap-1" action="{{ route('conciliacion.submaterias.update', $submateria->id) }}">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="w-full flex flex-col gap-1">
                                    <div class="relative z-0 w-full mb-6 group">
                                        <select name="materia" id="materia" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                                            @if($submateria->materia == 'Civil')
                                            <option value="Civil">{{ $submateria->materia }}</option>
                                            <option value="Familia">Familia</option>
                                            <option value="Contrataciones con el estado">Contrataciones con el estado</option>
                                            @endif
                                            @if ($submateria->materia == 'Contrataciones con el estado')
                                            <option value="Contrataciones con el estado">{{ $submateria->materia }}</option>
                                            <option value="Civil">Civil</option>
                                            <option value="Familia">Familia</option>
                                            @endif
                                            @if ($submateria->materia == 'Familia')
                                            <option value="Familia">{{ $submateria->materia }}</option>
                                            <option value="Civil">Civil</option>
                                            <option value="Contrataciones con el estado">Contrataciones con el estado</option>
                                            @endif
                                        </select>
                                        <label for="materia" class="after:content-['*'] after:text-red-400 peer-focus:font-medium absolute text-sm text-gray-500 transform -translate-y-6 scale-90 top-3 -z-10 origin-[0] peer-focus:left-0 duration-300 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-90 peer-focus:-translate-y-6">Materias</label>
